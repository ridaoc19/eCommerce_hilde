...
const DepartmentList: React.FC<DepartmentListProps> = ({ departmentList, isLoading, handleOnClick }) => {
  const { dashboard: { dispatch: dispatchContext, state: { inventory: { department_id }, permits: { admin } } } }: IContext.IContextData = useContext(CreateContext)!;
  ...
  return (
    <div>
      ...
      {department_id && admin && <button disabled={isLoading} name={ButtonName.Add} onClick={handleOnClick}>Nuevo Departamento</button>}
    </div >
  );
}
