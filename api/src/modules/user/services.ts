...
export async function postReset(req: Request, res: Response) {
  try {
    ...
    if (!userDB) throw new Error(`errorString: <p>Lo sentimos, el usuario (<span>${req.body.email}</span>) no está registrado en nuestra página. Por favor, verifique que ha ingresado correctamente su correo electrónico o regístrese para crear una nueva cuenta.</p>`)
