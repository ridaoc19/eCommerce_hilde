...
export async function postRegistre(req: Request, res: Response) {
  try {
    ...
    const userDB = await User.create({ name: req.body.name, lastName: req.body.lastName, email: req.body.email, password, state: false })
    ...
    const { _id, name, email, state } = userDB;

    const responseEmail: boolean = await sendEmail({ name, email, password: temporaryPassword })
    ...
    res.status(200).json({ _id, name, email, state })
...
export async function postLogin(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({ _id, name, lastName, email, state, token })
...
export async function postReset(req: Request, res: Response) {
  try {
    ...
    const temporaryPassword: string = uuidv4().split("-", 1)[0];
    const password = await generateHashPassword(temporaryPassword)
    const userUpdateDB = await User.findByIdAndUpdate(userDB._id, { password, state: false }, { new: true })
    ...
    const { _id, name, lastName, email, state } = userUpdateDB;
    const user = await fetchCount({ _id, name })
...
export async function postPassChange(req: Request, res: Response) {
  try {
    ...
    const userDB = await User.findByIdAndUpdate({ _id: idFront }, { password, state: true }, { new: true })
